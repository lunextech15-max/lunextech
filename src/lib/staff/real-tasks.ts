...
// public.task_checklist_items / public.task_comments. Replaces MOCK_TASKS
// (tasks-data.ts). RLS (0011_tasks.sql): any authenticated user can view
// the whole board.

import { createClient } from "@/lib/supabase/server";
import type { ChecklistItem, Task, TaskComment, TaskPriority, TaskStatus } from "./types";
...
async function assemble(rows: TaskRow[]): Promise<Task[]> {
  ...
  const assigneeIds = Array.from(new Set(rows.map((r) => r.assignee_staff_id).filter((id): id is string => Boolean(id))));
  const projectCodes = Array.from(new Set(rows.map((r) => r.project_code).filter((c): c is string => Boolean(c))));

  const [{ data: checklistRows }, { data: commentRows }, { data: staffRows }, { data: projectRows }] =
    await Promise.all([
      ...
      assigneeIds.length
        ? supabase.from("staff").select("staff_id, full_name").in("staff_id", assigneeIds)
        : Promise.resolve({ data: [] as StaffNameRow[] }),
      projectCodes.length
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      assigneeName: row.assignee_staff_id ? nameByStaffId.get(row.assignee_staff_id) ?? row.assignee_staff_id : "Unassigned",
