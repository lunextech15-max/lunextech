...
import { STATUS_LABEL, type AttendanceRecord, type AttendanceStatus } from "@/lib/staff/attendance-types";
...
function toCsv(records: AttendanceRecord[]): string {
  const header = "Name,Staff ID,Date,Status,Check-in,Check-out,Notes";
  const rows = records.map((r) =>
    [r.name, r.staffId, r.date, STATUS_LABEL[r.status], r.checkIn ?? "", r.checkOut ?? "", r.notes]
    ...
    <div className="px-6 py-10 md:px-10 lg:px-16 lg:py-14">
      <div className="dash-fade flex flex-wrap items-start justify-between gap-6">
        <div>
          <p className="text-[11px] font-medium tracking-[0.25em] text-soft-white/40 uppercase">
            05 <span className="text-accent">/ Attendance</span>
          </p>
          ...
          <p className="mt-3 text-sm text-soft-white/50 sm:text-base">
            Team and intern attendance overview.
          </p>
        </div>
      ...
      <div className="att-summary-grid dash-fade mt-8" style={{ animationDelay: "0.06s" }}>
        <div className="att-summary-tile">
          <p className="att-summary-value">{String(counts.present).padStart(2, "0")}</p>
          <p className="att-summary-label">Present today&apos;s log</p>
        </div>
        <div className="att-summary-tile">
          <p className="att-summary-value">{String(counts.late).padStart(2, "0")}</p>
          <p className="att-summary-label">Late</p>
        </div>
        <div className="att-summary-tile">
          <p className="att-summary-value">{String(counts.absent).padStart(2, "0")}</p>
          <p className="att-summary-label">Absent</p>
        </div>
        <div className="att-summary-tile">
          <p className="att-summary-value">{String(counts.onLeave).padStart(2, "0")}</p>
          <p className="att-summary-label">On leave</p>
        </div>
      </div>
      ...
      <div className="dash-fade mt-8" style={{ animationDelay: "0.18s" }}>
        {filtered.length > 0 ? (
          <div className="border border-line px-6 sm:px-8">
            {filtered.map((record) => (
              <div key={record.id} className="att-table-row">
