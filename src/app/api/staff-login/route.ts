...
// That RPC was a rate-limited but real enumeration + PII-disclosure
// oracle (see 0006_login_hardening.sql's own comments). This Route
// Handler does the same resolution server-side instead, using the
// service-role key (admin.ts) — the client never receives a raw email,
// only a generic success/failure and, on success, the role to redirect
// by. The RPC itself is revoked from anon/authenticated in
// 0017_login_rpc_hardening.sql; this replaces it entirely.

import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { createAdminClient } from "@/lib/supabase/admin";

const GENERIC_ERROR = "Invalid Staff/Intern ID or password.";
const RATE_LIMIT_WINDOW_MINUTES = 15;
const RATE_LIMIT_MAX_ATTEMPTS = 10;
// A real login goes through signInWithPassword either way (even for an ID
// that doesn't exist, against a fixed nonexistent address) so response
// timing doesn't itself reveal whether a given Staff ID is real.
